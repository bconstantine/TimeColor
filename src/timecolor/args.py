...
class BaseArgs:
    ...
    resume_from_checkpoint (`str`, defaults to `None`):
        ...
        in the `--output_dir`.
    stage ('int', defaults to `None`):
        Stage of the training. This is useful for multi-stage training, where each stage has different hyperparameters.
    rope_enumeration_method ('str', defaults to `None`):
        Method to use for enumerating the ROPE positions. This is useful for models that use ROPE embeddings.
    ...
    combine_multistage_datasets_with_ratio: List[int] = []
    ...
    use_recolor_mask: bool = False
    no_mask_ablation: bool = False
    just_beginning_new_stage: bool = False
    ...
    force_every_validation_to_be_stage2_latent: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        model_arguments = {
            ...
            "custom_transformer_path": self.custom_transformer_path,
        ...
        dataset_arguments = {
            ...
            "combine_multistage_datasets_with_ratio": self.combine_multistage_datasets_with_ratio,
            ...
            "use_recolor_mask": self.use_recolor_mask,
            "no_mask_ablation": self.no_mask_ablation,
            "just_beginning_new_stage": self.just_beginning_new_stage,
        }
        ...
        training_arguments = {
            ...
            "stage": self.stage,
            "attention_mask_stage": self.attention_mask_stage,
            "rope_enumeration_method": self.rope_enumeration_method,
        }
        ...
        validation_arguments = {
            ...
            "force_every_validation_to_be_stage2_latent": self.force_every_validation_to_be_stage2_latent,
        }
...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--custom_transformer_path", type=str, default=None, help="Path to a custom transformer model.")
...
def _add_dataset_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--combine_multistage_datasets_with_ratio",
        type=int,
        nargs="+",
        default=[],
        help="If provided, will combine multiple datasets with the given ratio. The ratio must be a list of integers, where the sum of the integers must be equal to the number of datasets.",)
    ...
    parser.add_argument("--use_recolor_mask", action="store_true")
    parser.add_argument("--no_mask_ablation", action="store_true")
    parser.add_argument("--just_beginning_new_stage", action="store_true",)
...
def _add_training_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--training_type", type=str, choices=[x.value for x in TrainingType.__members__.values()], required=False
    )
    ...
    parser.add_argument("--attention_mask_stage", type=int, default=3)
    parser.add_argument(
        "--rope_enumeration_method",
        type=str,
        default="",
        help="Method to use for enumerating the ROPE positions. This is useful for models that use ROPE embeddings.",
    )
...
def _add_validation_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--force_every_validation_to_be_stage2_latent", action="store_true")
...
def _map_to_args_type(args: Dict[str, Any]) -> BaseArgs:
    ...
    result_args.custom_transformer_path = args.custom_transformer_path
    ...
    result_args.combine_multistage_datasets_with_ratio = args.combine_multistage_datasets_with_ratio
    if isinstance(result_args.combine_multistage_datasets_with_ratio, list):
        result_args.combine_multistage_datasets_with_ratio = [int(x) for x in result_args.combine_multistage_datasets_with_ratio]
    if isinstance(result_args.combine_multistage_datasets_with_ratio, int):
        result_args.combine_multistage_datasets_with_ratio = [result_args.combine_multistage_datasets_with_ratio]
    ...
    result_args.use_recolor_mask = args.use_recolor_mask
    result_args.no_mask_ablation = args.no_mask_ablation
    result_args.just_beginning_new_stage = args.just_beginning_new_stage
    ...
    result_args.stage = args.stage
    result_args.attention_mask_stage = args.attention_mask_stage
    result_args.rope_enumeration_method = args.rope_enumeration_method
    ...
    result_args.force_every_validation_to_be_stage2_latent = args.force_every_validation_to_be_stage2_latent
...
def _validate_dataset_args(args: BaseArgs):
    ...
    assert isinstance(args.combine_multistage_datasets_with_ratio, list), (
        "Combine multistage datasets with ratio must be a list of integers."
    )
    if args.combine_multistage_datasets_with_ratio and not all(
        isinstance(x, int) and x > 0 for x in args.combine_multistage_datasets_with_ratio
    ):
        raise ValueError("All elements in combine_multistage_datasets_with_ratio must be positive integers.")
...
def _validate_validation_args(args: BaseArgs):
    ...
    logger.info("force_every_validation_to_be_stage2_latent at the moment is: ")
    logger.info(args.force_every_validation_to_be_stage2_latent)
