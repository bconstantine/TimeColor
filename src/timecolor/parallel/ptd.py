...
from timecolor.data import DPDataLoader
from timecolor.logging import get_logger
from timecolor.utils import enable_determinism, get_device_info
from timecolor.utils._common import DIFFUSERS_TRANSFORMER_BLOCK_NAMES
from torch.distributed.checkpoint import DefaultLoadPlanner
from .base import BaseCheckpointer, BaseParallelBackend


if TYPE_CHECKING:
    from timecolor import optimizer
...
class FlexibleLoaderList(torch.distributed.checkpoint.stateful.Stateful):
    """
    Wraps a list[DPDataLoader] so that:
      • Missing loaders in the *checkpoint* are ignored.
      • Extra loaders in the *checkpoint* (that the current run no longer has)
        are also ignored.
    """
    def __init__(self, loaders):
        self.loaders = loaders                # keep original object!

    # --------------  Required by Stateful  -------------- #
    def state_dict(self, destination=None, prefix='', metadata=None):
        # Save each loader under a *string* key
        return {str(i): dl.state_dict() for i, dl in enumerate(self.loaders)}

    def load_state_dict(self, state_dict, prefix='', metadata=None):
        for k, sd in state_dict.items():
            idx = int(k)
            if idx < len(self.loaders):       # loader still exists → restore
                self.loaders[idx].load_state_dict(sd)
            # else: loader was removed → ignore
        # Any *new* loaders (indices ≥ len(state_dict)) keep their fresh state

    # Optional but nice: keep the underlying list interface
...
class PTDCheckpointer(BaseCheckpointer):
    def __init__(
        ...
        dataloader_list: Optional[List[torch.utils.data.DataLoader]] = None,
    ) -> None:
        self.states = states
        #TODO(Bryan): Save the dataloaders list"
        if dataloader_list is not None:
            if dataloader is not None:
                raise ValueError("Cannot provide both a single dataloader and a list of dataloaders.")
            # Wrap so mismatched lengths won't crash load()
            self.states["dataloader_list"] = FlexibleLoaderList(dataloader_list)
        else:
            if dataloader is None:
                raise ValueError("Either a single dataloader or a list of dataloaders must be provided.")
    ...
    #TODO(Bryan): Refactor this, do not hardcode the just beginning new stage
    def load(self, step: int = -1, just_beginning_new_stage: bool = False) -> bool:
        if not self.enable:
            return False
        ...
        if just_beginning_new_stage:
            original_dl = self.states.pop("dataloader", None)
            original_dl_list = self.states.pop("dataloader_list", None)
        ...
        self._dcp_load_with_fallback(states, checkpoint_dir)
        ...
        if just_beginning_new_stage:
            if original_dl is not None:
                self.states["dataloader"] = original_dl
            if original_dl_list is not None:
                self.states["dataloader_list"] = original_dl_list
        return True
    ...
    def _dcp_load_with_fallback(self, states, checkpoint_dir):
        """
        Try to load.  If the checkpoint was created by older code that
        didn't save `dataloader_list`, remove it from `states` and try again.
        """
        try:
            print("Entering main load path")
            torch.distributed.checkpoint.load(
                states, checkpoint_id=checkpoint_dir.as_posix()
            )
            print("Loading finished successfully")
        except (RuntimeError, CheckpointException) as e:
            msg = str(e)
            print("Entering runtime error handling")
            if "dataloader_list" not in msg:
                raise                       # unrelated error → bail out

            # Fallback path --------------------qqqqqqq------------------------------
            print("Entering fallback error handling")
            if "dataloader_list" in states:
                logger.warning(
                    "Checkpoint has no `dataloader_list`; loading without it "
                    "and starting loaders from epoch 0."
                )
                states.pop("dataloader_list")
