...
from timecolor.parallel import ParallelBackendEnum
from timecolor.processors import ProcessorMixin
from timecolor.typing import ArtifactType, SchedulerType, TokenizerType
from timecolor.utils import resolve_component_cls
...
class ModelSpecification:
    ...
    def prepare_conditions_preencoded(self) -> Dict[str, torch.nn.Module]:
        raise NotImplementedError(
            f"ModelSpecification::prepare_conditions_preencoded is not implemented for {self.__class__.__name__}"
        )

    def prepare_latents_preencoded(self) -> Dict[str, torch.nn.Module]:
        raise NotImplementedError(
            f"ModelSpecification::prepare_latents_preencoded is not implemented for {self.__class__.__name__}"
        )
