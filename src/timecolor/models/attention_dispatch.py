...
from timecolor.constants import FINETRAINERS_ATTN_CHECKS, FINETRAINERS_ATTN_PROVIDER
from timecolor.logging import get_logger
from timecolor.utils.import_utils import (
...
def _finetrainers_flash_attn_forward_newers(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    dropout_p: float = 0.0,
    ...
    # 1) reorder to [B, S, N, D] and make contiguous
    q_contig = query.permute(0, 2, 1, 3).contiguous()
    k_contig = key.permute(0, 2, 1, 3).contiguous()
    v_contig = value.permute(0, 2, 1, 3).contiguous()

    # 2) compute a sane default if no scale is provided
    d = q_contig.shape[-1]
    softmax_scale = scale if scale is not None else 1.0 / math.sqrt(d)

    # 3) unpack your window‐size tuple
    window_size_left, window_size_right = window_size

    # 4) call the new op
    #    returns: (out_raw, softmax_lse, S_dmask, rng_state)
    out_raw, softmax_lse, S_dmask, rng_state = _flash_attn_forward(
        q_contig,
        k_contig,
        v_contig,
        dropout_p,
    ...
    # 5) undo the permutation for your final output
    out = out_raw.permute(0, 2, 1, 3).contiguous()

    # 6) for compatibility, return the same 8‐tuple:
    #    (out, softmax_lse, q, k, v, out_padded, S_dmask, rng_state)
    #    here we reuse `out_raw` as the “padded” output
    out_padded = out_raw

    return out, softmax_lse, q_contig, k_contig, v_contig, out_padded, S_dmask, rng_state
...
def _finetrainers_flash_attn_backward_newers(
    grad_out: torch.Tensor,                # [B, N, S, D]
    query: torch.Tensor,                   # [B, N, S, D]
    key: torch.Tensor,                     # [B, N, S, D]
    value: torch.Tensor,                   # [B, N, S, D]
    out: torch.Tensor,                     # [B, N, S, D]
    logsumexp: torch.Tensor,               # [B, S, N] (from forward)
    dropout_p: float,
    scale: Optional[float] = None,
    is_causal: bool = False,
    window_size: Tuple[int, int] = (-1, -1),
    ...
    # 1) permute into [B, S, N, D]
    dout = grad_out.permute(0, 2, 1, 3).contiguous()
    ...
    v = value.contiguous()
    #outc = out.permute(0, 2, 1, 3).contiguous()
    outc = out.contiguous()
    lse  = logsumexp  # already [B, S, N]
    ...
    # 3) compute default softmax_scale if needed
    D = q.shape[-1]
    softmax_scale = scale if scale is not None else 1.0 / math.sqrt(D)

    # 4) unpack window‐size
    window_left, window_right = window_size

    # 5) call the new backward op (fills dq,dk,dv in‐place)
    _flash_attn_backward(
        dout, q, k, v, outc, lse,
        dq, dk, dv,
        dropout_p, softmax_scale, is_causal,
        window_left, window_right,
        softcap, alibi_slopes,
        deterministic, rng_state,
    )

    # 6) remove any padded heads
    dq = dq[..., :D]
    dk = dk[..., :D]
    dv = dv[..., :D]

    # 7) permute back to [B, N, S, D]
    if _permute_outputs:
...
class _flash_attn_flash_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        out, lse, q, k, v, out_padded, S_dmask, rng_state = _finetrainers_flash_attn_forward_newers(
            query=q,
            key=k,
            value=v,
            dropout_p=dropout_p,
    ...
    def backward(
        ...
        *args: torch.Tensor,
        ...
        grad_query, grad_key, grad_value = _finetrainers_flash_attn_backward_newers(
            grad_out=grad_out,
            query=q,
            key=k,
            value=v,
...
class _native_ring_flash_attn_flash_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        out, lse, q, k, v, out_padded, S_dmask, rng_state = _templated_ring_attention(
            ...
            seq_dim=2,
            op=_finetrainers_flash_attn_forward_newers,
            query=q,
            key=k,
            value=v,
            dropout_p=dropout_p,
    ...
    def backward(
        ...
        *args: torch.Tensor,
        ...
        grad_query, grad_key, grad_value = _templated_ring_attention_backward(
            ...
            # returned outputs
            seq_dim=1,
            op=functools.partial(_finetrainers_flash_attn_backward_newers, _permute_outputs=False),
            grad_out=grad_out,
