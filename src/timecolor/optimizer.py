...
class OptimizerWrapper(Stateful):
    ...
    #TODO (Bryan): Casting optimizer to required dtype
    def cast_state_to_param_dtype(self) -> None:
        """
        After a checkpoint is loaded, make sure every running-average buffer
        (exp_avg, exp_avg_sq, …) has the *same dtype* as its parameter.
        Call this once before the next optimizer.step().
        """
        for opt in self.optimizers:
            for group in opt.param_groups:
                for p in group["params"]:
                    state = opt.state[p]
                    for buf in ("exp_avg", "exp_avg_sq", "exp_avg_hat"):
                        if buf in state and torch.is_tensor(state[buf]):
                            #hardcode them to float32
                            pass
                            #state[buf] = state[buf].to(torch.float32)
    #TODO (Bryan): Casting optimizer to required dtype
    def cast_state_to_param_dtype(self) -> None:
        """
        After a checkpoint is loaded, make sure every running-average buffer
    ...
    #TODO (Bryan): solve different type of parameters from training pipeline
    def eager_initialize_adam_state(self) -> None:
        """
        Make sure every trainable parameter already has its exp_avg /
        exp_avg_sq buffers, and make them match the parameter dtype.
        Call this *after* casting the model to bf16 and *before* the first
        optimizer.step().
        """
        for opt in self.optimizers:
            for group in opt.param_groups:
                for p in group["params"]:
                    #force init if it doesnt exist
                    st  = opt.state[p]
