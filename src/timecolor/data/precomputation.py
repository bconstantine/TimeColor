...
from timecolor.utils import delete_files
import os
...
) -> Union["InMemoryDistributedDataPreprocessor", "PrecomputedDistributedDataPreprocessor", "PreencodedDistributedDataPreprocessor"]:
    if enable_precomputation:
        #TODO(Bryan): Change this to a better way to handle preencoded
        # return PreencodedDistributedDataPreprocessor(
    ...
    def consume(
        ...
        if not self._enable_reuse:
            ...
            for i in tqdm(range(self._num_items), desc=f"Rank {self._rank}", total=self._num_items):
                ...
                # TODO(Bryan): CHANGE THE _save_item FUNCTION TO DEFAULT USE CASE
...
class PreencodedDistributedDataPreprocessor(DistributedDataProcessorMixin):
    def __init__(
        self,
        rank: int,
        world_size: int,
        ...
        self._world_size = world_size
        self._num_items = num_items
        print("num items here are: ", self._num_items) #correspond to how many data to load at once, perdevice
        self._processor_fn = processor_fn
        self._save_dir = pathlib.Path(save_dir) 
        self._enable_reuse = enable_reuse

        self._cached_samples = []
        self._preprocessed_iterator: Union["PrecomputedDataIterable", "PrecomputedOnceDataIterable", "PreencodedDataIterable"] = None

        assert enable_reuse, "enable reuse must be True for PreencodedDistributedDataPreprocessor"

        #make self dir if not exists
    ...
    def consume(
        ...
        if cache_samples:
            ...
            if drop_samples:
                raise ValueError("Cannot cache and drop samples at the same time.")
        #TODO (Bryan): Donot hardcode this
        for i in tqdm(range(self._num_items), desc=f"Rank {self._rank}", total=self._num_items):
            if use_cached_samples:
                item = self._cached_samples[i]
            else:
        ...
        #TODO (Bryan): Solve this hardcoded issue
        # if self._enable_reuse:
        #     data_iterator = PrecomputedOnceDataIterable(self._rank, self._world_size, self._save_dir, data_type)
        # else:
        #     data_iterator = PrecomputedDataIterable(self._rank, self._world_size, self._save_dir, data_type)
        ...
        #cache the required data name to the local txt
        data_iterator = PreencodedDataIterable(self._rank, self._world_size, self._save_dir, data_type, data_amount=self._num_items)
        #TODO (Bryan): DO NOT HARDCODE THE NUM ITEMS
        data_iterator._num_items = self._num_items
    ...
    def consume_once(
        ...
        #TODO (Bryan): Donot hardcode this
        raise ValueError("PreencodedDistributedDataPreprocessor does not support consume_once method.")
        if data_type not in self._processor_fn.keys():
            raise ValueError(f"Invalid data type: {data_type}. Supported types: {list(self._processor_fn.keys())}")
        if cache_samples:
            if use_cached_samples:
...
class PreencodedDataIterable:
    ...
    def __init__(self, rank: int, world_size: int, save_dir: str, data_type: str, data_amount:int) -> None:
        self._rank = rank
        self._world_size = world_size
        self._save_dir = pathlib.Path(save_dir)
        self._data_type = data_type
