...
from pathlib import Path
from timecolor.models.cogvideox.utils import prepare_rotary_positional_embeddings_cogvideox, concatenate_freqs_cos_freqs_sin_temporally
...
def implement_rope_custom(transformer_config,
...
def parse_common_args():
    ...
    p.add_argument("--work_json_path", type=str, default="./examples/inference_samples_xdit.json")
...
def setup_cfg_parallel(classifier_free_guidance_degree: int = 2):
    dist.init_process_group("nccl")
    init_distributed_environment(rank=dist.get_rank(), world_size=dist.get_world_size())
    initialize_model_parallel(classifier_free_guidance_degree=classifier_free_guidance_degree)
...
if __name__ == "__main__":
    ...
    local_rank, device, is_rank0 = setup_cfg_parallel(classifier_free_guidance_degree=2)
    dtype = torch.bfloat16
