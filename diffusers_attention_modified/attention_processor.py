...
class Attention(nn.Module):
    ...
    def forward(
        ...
        attention_mask_level: int = 0,
        mask_tokens_range: Optional[torch.Tensor] = None,
        **cross_attention_kwargs,
        ...
        return self.processor(
            ...
            attention_mask_level=attention_mask_level,
            mask_tokens_range=mask_tokens_range,
            **cross_attention_kwargs,
        )
...
Segments = Dict[str, object]  # name -> (start, end) over concat dim S


def build_allow(level: int, segs: Segments) -> Dict[str, List[str]]:
    """
    Segment-level routing rules.
    NOTE: level 4 needs pixel-level routing, so at segment level it matches level 3.
    """
    assert level in (0, 1, 2, 3, 4), f"Invalid level: {level}"

    # IMPORTANT: segs may include "PIXEL_PACK" (not a span)
    names = [n for n, v in segs.items() if isinstance(v, tuple) and len(v) == 2]

    allow: Dict[str, List[str]] = {n: list(names) for n in names}

    if level == 0:
        return allow

    ref_names = [n for n in names if n.startswith("REF_")]
    ...
    if level == 3:
        ...
        for r in ref_names:
            allow[r] = [n for n in names if (n not in ref_set) or (n == r)]
        return allow

    if level == 4:
        # Segment-level: same as level 3 (refs can't see other refs).
        # Pixel-level ref alignment is handled inside routed_sdpa(level=4).
        ref_set = set(ref_names)
        for r in ref_names:
            allow[r] = [n for n in names if (n not in ref_set) or (n == r)]
        return allow
...
#     segs: Segments,   # {"TEXT":(0,226), "MAIN":(...), "SKETCH":(...), "MASK":(...), "REF_0":(...), ...}
#     level: int,
#     dropout_p: float = 0.0,
#     is_causal: bool = False,
# ) -> torch.Tensor:
#     """
#     Applies segment-wise routed attention without an attn_mask,
#     so Flash/ME kernels stay enabled.

#     `level` here is the mask level (0..3), *not* the pipeline stage.
#     """
#     assert q.dim() == k.dim() == v.dim() == 4 and q.shape == k.shape == v.shape
#     B, H, S, D = q.shape

#     # Fast exit: level 0 == full attention
#     if level == 0:
#         return F.scaled_dot_product_attention(
#             q, k, v, dropout_p=dropout_p, is_causal=is_causal
#         )

#     allow = build_allow(level, segs)
#     ordered = list(segs.items())  # [(name, (s,e)), ...]

#     # Cache concatenated K/V per unique allow-set (to avoid repeated cat)
#     kv_cache: Dict[Tuple[str, ...], Tuple[torch.Tensor, torch.Tensor]] = {}

#     outs: List[Tuple[int, int, torch.Tensor]] = []
#     full_key = tuple([n for n, _ in ordered])

#     for name, (qs, qe) in ordered:
#         Q = q[:, :, qs:qe, :]  # [B, H, Q, D]
#         allowed_names = tuple(allow[name])

#         if allowed_names == full_key:
#             # can see everyone: reuse original K/V
#             K_allowed, V_allowed = k, v

#         elif len(allowed_names) == 1 and allowed_names[0] == name:
#             # self-only
#             ks, ke = segs[name]
#             K_allowed = k[:, :, ks:ke, :]
#             V_allowed = v[:, :, ks:ke, :]
...
#         else:
#             # some subset: reuse or build concatenated K/V
#             if allowed_names not in kv_cache:
#                 K_allowed = torch.cat(
#                     [k[:, :, segs[n][0]:segs[n][1], :] for n in allowed_names],
#                     dim=2,
#                 )
#                 V_allowed = torch.cat(
#                     [v[:, :, segs[n][0]:segs[n][1], :] for n in allowed_names],
#                     dim=2,
#                 )
#                 kv_cache[allowed_names] = (K_allowed, V_allowed)
#             else:
#                 K_allowed, V_allowed = kv_cache[allowed_names]

#         O = F.scaled_dot_product_attention(
#             Q, K_allowed, V_allowed,
#             attn_mask=None, dropout_p=dropout_p, is_causal=is_causal
#         )
#         outs.append((qs, qe, O))

#     outs.sort(key=lambda t: t[0])
#     return torch.cat([o for _, _, o in outs], dim=2)

@torch._dynamo.disable
def routed_sdpa(
    q: torch.Tensor,  # [B, Hh, S, D]
    k: torch.Tensor,  # [B, Hh, S, D]
    v: torch.Tensor,  # [B, Hh, S, D]
    segs: Segments,   # spans + optional "PIXEL_PACK"
    level: int,
    dropout_p: float = 0.0,
    is_causal: bool = False,
) -> torch.Tensor:
    """
    Applies routed attention WITHOUT attn_mask (Flash/ME kernels stay enabled).
    For level 4: pixel-level routing using segs["PIXEL_PACK"].
    """
    assert q.dim() == k.dim() == v.dim() == 4 and q.shape == k.shape == v.shape
    B, Hh, S, D = q.shape

    # Fast exit: full attention
    if level == 0:
        return F.scaled_dot_product_attention(q, k, v, dropout_p=dropout_p, is_causal=is_causal)

    # Filter segment spans (ignore PIXEL_PACK, etc.)
    span_segs: Dict[str, Tuple[int, int]] = {
        n: vv for n, vv in segs.items()
        if isinstance(vv, tuple) and len(vv) == 2
    }

    if level in (1, 2, 3):
        allow = build_allow(level, span_segs)
        ordered = list(span_segs.items())  # [(name,(s,e)), ...]
        ...
        for name, (qs, qe) in ordered:
            ...
            if allowed_names == full_key:
                ...
                ks, ke = span_segs[name]
                K_allowed = k[:, :, ks:ke, :]
                V_allowed = v[:, :, ks:ke, :]

            else:
                if allowed_names not in kv_cache:
                    K_allowed = torch.cat([k[:, :, span_segs[n][0]:span_segs[n][1], :] for n in allowed_names], dim=2)
                    V_allowed = torch.cat([v[:, :, span_segs[n][0]:span_segs[n][1], :] for n in allowed_names], dim=2)
                    kv_cache[allowed_names] = (K_allowed, V_allowed)
                else:
                    K_allowed, V_allowed = kv_cache[allowed_names]

            O = F.scaled_dot_product_attention(Q, K_allowed, V_allowed, attn_mask=None, dropout_p=dropout_p, is_causal=is_causal)
            outs.append((qs, qe, O))
    ...
    # ----------------------------
    # Level 4: pixel-aligned routing
    # ----------------------------
    if level != 4:
        raise ValueError(f"Unsupported level: {level}")
    if "PIXEL_PACK" not in segs:
        raise RuntimeError("level=4 requires segs['PIXEL_PACK'] (output of build_attention_pixel_pack)")

    pixel_pack = segs["PIXEL_PACK"]
    if not isinstance(pixel_pack, dict):
        raise RuntimeError("segs['PIXEL_PACK'] must be a dict (from build_attention_pixel_pack)")

    # Packed pixel routing structures (already on device ideally)
    sorted_local_idx: torch.Tensor = pixel_pack["sorted_local_idx"]  # int32, len N
    ptr: torch.Tensor = pixel_pack["ptr"]                            # int32, len K+2
    allowed_ids = pixel_pack.get("allowed_ids", (0, 1, 2, 3))
    allowed_ids = [int(x) for x in allowed_ids]
    Kmax = int(pixel_pack.get("K", max(allowed_ids) if allowed_ids else 0))
    N = int(pixel_pack["N"])

    # Helper: segments that are always visible (non-ref blocks)
    common_names = ["TEXT", "MAIN", "SKETCH"]
    if "MASK" in span_segs:
        common_names.append("MASK")

    # 2) TEXT can see all refs (i.e., full attention). We just treat TEXT normally: K/V = full.
    # 1) Each REF_i can only see common + itself (no other refs).
    # 3) Pixel blocks (MAIN/SKETCH/MASK) see common + REF_label (aligned by pixel label).

    # Build common K/V once: concat TEXT+MAIN+SKETCH(+MASK)
    def _cat_kv(names: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        Kc = torch.cat([k[:, :, span_segs[n][0]:span_segs[n][1], :] for n in names], dim=2)
        Vc = torch.cat([v[:, :, span_segs[n][0]:span_segs[n][1], :] for n in names], dim=2)
        return Kc, Vc

    K_common, V_common = _cat_kv(common_names)

    # Cache K/V per label (common + REF_label if exists)
    kv_for_label: Dict[int, Tuple[torch.Tensor, torch.Tensor]] = {}

    def _get_kv_for_label(lbl: int) -> Tuple[torch.Tensor, torch.Tensor]:
        if lbl in kv_for_label:
            return kv_for_label[lbl]

        ref_name = f"REF_{lbl}"
        if ref_name in span_segs:
            rs, re = span_segs[ref_name]
            K_ref = k[:, :, rs:re, :]
            V_ref = v[:, :, rs:re, :]
            K_allowed = torch.cat([K_common, K_ref], dim=2)
            V_allowed = torch.cat([V_common, V_ref], dim=2)
        else:
            # If corresponding ref doesn't exist: fall back to common only
            K_allowed, V_allowed = K_common, V_common

        kv_for_label[lbl] = (K_allowed, V_allowed)
        return K_allowed, V_allowed

    # Cache K/V per REF query segment (common + itself)
    kv_for_refseg: Dict[str, Tuple[torch.Tensor, torch.Tensor]] = {}

    def _get_kv_for_refseg(ref_name: str) -> Tuple[torch.Tensor, torch.Tensor]:
        if ref_name in kv_for_refseg:
            return kv_for_refseg[ref_name]
        rs, re = span_segs[ref_name]
        K_ref = k[:, :, rs:re, :]
        V_ref = v[:, :, rs:re, :]
        K_allowed = torch.cat([K_common, K_ref], dim=2)
        V_allowed = torch.cat([V_common, V_ref], dim=2)
        kv_for_refseg[ref_name] = (K_allowed, V_allowed)
        return K_allowed, V_allowed

    # Helper: run pixel-grid routing for one segment if it exists
    def _route_pixel_segment(seg_name: str) -> Optional[torch.Tensor]:
        if seg_name not in span_segs:
            return None
        qs, qe = span_segs[seg_name]
        if (qe - qs) != N:
            raise RuntimeError(
                f"{seg_name} span length {(qe-qs)} != pixel_pack.N {N}. "
                f"Check that {seg_name} is on the attention grid."
            )

        Qblk = q[:, :, qs:qe, :]                  # [B,Hh,N,D]
        Oblk = torch.empty_like(Qblk)             # [B,Hh,N,D]

        # Fill by label groups using compact index slices
        for lbl in allowed_ids:
            if lbl < 0 or lbl > Kmax:
                continue
            a = int(ptr[lbl].item())
            b = int(ptr[lbl + 1].item())
            if a == b:
                continue

            loc = sorted_local_idx[a:b].to(dtype=torch.long)   # local indices in [0..N)
            Qsub = torch.index_select(Qblk, dim=2, index=loc)  # [B,Hh,M,D]

            K_allowed, V_allowed = _get_kv_for_label(lbl)

            Osub = F.scaled_dot_product_attention(
                Qsub, K_allowed, V_allowed,
                attn_mask=None, dropout_p=dropout_p, is_causal=is_causal
            )
            # Scatter back
            Oblk.index_copy_(2, loc, Osub)

        return Oblk

    # Now assemble outputs in original segment order (by span start)
    ordered = sorted(span_segs.items(), key=lambda kv: kv[1][0])
    outs: List[Tuple[int, int, torch.Tensor]] = []

    for name, (qs, qe) in ordered:
        if name == "TEXT":
            Q = q[:, :, qs:qe, :]
            O = F.scaled_dot_product_attention(Q, k, v, attn_mask=None, dropout_p=dropout_p, is_causal=is_causal)
            outs.append((qs, qe, O))
            continue

        if name.startswith("REF_"):
            Q = q[:, :, qs:qe, :]
            K_allowed, V_allowed = _get_kv_for_refseg(name)
            O = F.scaled_dot_product_attention(Q, K_allowed, V_allowed, attn_mask=None, dropout_p=dropout_p, is_causal=is_causal)
            outs.append((qs, qe, O))
            continue

        if name in ("MAIN", "SKETCH", "MASK"):
            Oblk = _route_pixel_segment(name)
            if Oblk is None:
                # segment not present (e.g., MASK missing), skip
                continue
            outs.append((qs, qe, Oblk))
            continue

        # Fallback: if any unknown segment appears, keep old default = full
        Q = q[:, :, qs:qe, :]
        O = F.scaled_dot_product_attention(Q, k, v, attn_mask=None, dropout_p=dropout_p, is_causal=is_causal)
        outs.append((qs, qe, O))
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        #print("Entering attn processor of cogvideox")
        ...
        if attention_mask_level >= 1:
            hidden_states = routed_sdpa(
                query,
                key,
                value,
                segs=mask_tokens_range,
                level=attention_mask_level,
                dropout_p=0.0,
                is_causal=False,
            )
        elif attention_mask_level == 0:
            hidden_states = F.scaled_dot_product_attention(
                query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
            )
        else:
            raise ValueError(
                f"Unsupported attention_mask_level: {attention_mask_level}. Supported values are 0 and 1."
            )
